...
### 当前进度 (Status)
- **黄历增强**: 基于 `tyme4ts` 能力，在 `getAlmanacInfo` 中新增了**值神、星宿、纳音、九星、月相、彭祖百忌**等高级参数，并在 `AlmanacCard` 中进行了展示，显著提升了黄历的专业度和丰富度。
- **UI 优化**: 优化了 `BirthInfoForm.vue` 在移动端 (max-width: 480px) 的网格布局，改善了输入框显示拥挤的问题。
- **完成真太阳时 (True Solar Time)**: 已引入经度校正功能 (`getTrueSolarTime`)，更新了八字计算核心逻辑及 Store/UI，支持用户输入出生地经度和分钟，确保排盘更精准。
- **完成 V2.1 算法优化**: 评分阈值已调整为友好模式（平>=45, 凶>=25），文档与代码已同步。
- **测试通过**: `src/utils/tyme.spec.js` 和 `src/utils/tyme.solar.spec.js` 全部通过。

### 关键记录 (Logs/Issues)
- **Almanac Update**: Enhanced `AlmanacCard.vue` to display Duty Officer, 28 Stars, NaYin, and 9 Stars.
- **UI Update**: Updated `BirthInfoForm.vue` CSS for mobile devices.
- **Feature Added**: Added `birthLongitude`, `birthMinute`, `currentLongitude` to User Store.
- **Algorithm Update**: `calculateBaZi` and `getCurrentHourBazi` now support TST correction.

### 下一步 (Next Steps)
- **UI 优化**: 在首页或详情页展示今日核心神煞（如图标）。
- **用户反馈监控**: 观察新算法和真太阳时功能上线后的用户反馈。
